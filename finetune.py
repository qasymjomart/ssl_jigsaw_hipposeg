...
from networks import JigsawUNetDown, UNet
from dataset import IXIdataset_Jigsaw, HarPdataset
...
# Pre-trained model path sample: home/guest/qasymjomart/seg_mri/hipposeg/checkpoints/coronal_jps_012_002000.pth.tar
...
parser.add_argument('--use_only_brain_slices', default=0, type=int, help='Whether to use slices containing brain or all slices (0 - no, 1 -yes)')
...
def main():
	...
	print("Label fractions: ", label_fractions)
	# TensorBoard setting
	timestamp_current = datetime.now()
	timestamp_current = timestamp_current.strftime("%Y%m%d_%H%M")
	writer = SummaryWriter(log_dir='/home/qasymjomart/runs/runs_jigsaw_finetune_' + args.savename + '_' + timestamp_current)

	# Here randomly pick subject indexes to be used for pre-training (90% of the total number of subjects, so subject-wise)
	# And save the subjects used in the txt file for the future fine-tuning on the subjects not selected for pre-training
	# Define the slices to be used for each view
	# Find the total number of subjects
	gtlists_data = natsorted(glob.glob(args.datapath + '*.nii'))
	...
	for label_fraction in label_fractions:
		...
		for finetune_type in ['linear', 'finetune', 'tfs']:
			...
			train_dataset = HarPdataset(
				...
				view=args.mri_view,
				mode='train',
				use_only_brain_slices = args.use_only_brain_slices
			)

			test_dataset = HarPdataset(
				dataroot=args.datapath,
				labelsroot=args.labelspath,
				indexes=test_idxs,
				view=args.mri_view,
			...
			for epoch in range(int(args.iter_start/iter_per_epoch),args.epochs):
				...
				for i, (images, masks) in enumerate(train_loader):
					...
					writer.add_scalar(finetune_type + '_' + str(label_fraction * 100) + '_' + args.savename + ' Jigsaw Dice coeff. K-Fold No. ', 1-train_dice, steps)
					writer.add_scalar(finetune_type + '_' + str(label_fraction * 100) + '_' + args.savename + ' Jigsaw Dice loss. K-Fold No. ', loss, steps)    
